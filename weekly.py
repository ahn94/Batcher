...
dfBeans = pd.read_excel(xls, sheet_name="Beans", converters={'IS': str, 'Batch#': int, 'Roast': str, 'Invoice': pd.to_datetime})
...
# print(tabulate(dfGreen, tablefmt='fancy_grid', floatfmt=('.0f','.0f','.0f','.0f','.0f','.0f','.0f','.0f','.0f','.0f','.0f','.2f')))

# print(dfGreen)

df = dfGreen[['Invoice','lb']].copy()

# print(df)

df = df.groupby(df['Invoice'].dt.to_period('W')).agg(np.sum)

print(tabulate(df, headers=['Date Range', 'green lb'],  tablefmt='fancy_grid', floatfmt=('.0f','.2f')))
